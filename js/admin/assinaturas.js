...
const AdminAssinaturas = (() => {
  ...
  function generateMockSubscriptions() {
    ...
    const statuses = ['ativa', 'ativa', 'ativa', 'ativa', 'ativa', 'pausada', 'cancelada', 'pendente'];
    const stores = ['centro', 'shopping', 'norte', 'sul', 'praia'];
    const subs = [];

    // Get products eligible for recurrence
    const eligibleProducts = DataProducts.filter(p => p.recorrencia && p.recorrencia.elegivel);
    if (eligibleProducts.length === 0) return subs;
    ...
    for (let i = 0; i < 20; i++) {
      ...
      while (histDate < new Date()) {
        historico.push({
          data: histDate.toISOString(),
          status: 'entregue',
          valor: variacao.preco * (1 - product.recorrencia.descontoPercent / 100),
        });
        histDate.setDate(histDate.getDate() + freq);
      }

      // If cancelled, set cancellation date
      let dataCancelamento = null;
      if (status === 'cancelada') {
        dataCancelamento = new Date(Date.now() - Math.floor(Math.random() * 30) * 86400000).toISOString();
      }
  ...
  function getSubscriptions() {
    let subs = Storage.get('subscriptions') || ensureMockData();
